...
export default function MockMenuPage() {
    const [status, setStatus] = useState("Ready to seed 50 Khmer menu items.");
    const [loading, setLoading] = useState(false);

    const handleSeed = async () => {
        if (!db) return alert("Database not initialized.");
        if (!confirm("This will write ~40 items to the database. Proceed?")) return;

        setLoading(true);
        setStatus("Starting batch write...");
        ...
        try {
            ...
            await batch.commit();
            setStatus("✅ Seed completed successfully! ~50 items loaded into Firestore.");
        } catch (e: any) {
            console.error(e);
    ...
    return (
        <div style={{ padding: "40px", fontFamily: "sans-serif", maxWidth: "600px" }}>
            <h1>Khmer Menu Seeder</h1>
            <p style={{ color: "var(--color-text-secondary)", marginBottom: "20px" }}>
                This will automatically create 6 categories, 36 traditional Khmer food/drink items, and 2 default schedules (All Day + Breakfast) directly into your live Firestore database.
            </p>
            ...
            <div style={{ padding: "16px", background: "var(--color-background-elevated)", borderRadius: "8px" }}>
                <strong>Status:</strong> {status}
            </div>

            <div style={{ marginTop: "30px" }}>
                <a href="/admin/menu" style={{ color: "var(--color-primary)", textDecoration: "underline" }}>
